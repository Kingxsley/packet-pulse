...
Examples:
    ...
    python run.py --dataset both --no-autoencoder
"""
import argparse
import json
...
def main():
    ...
    parser.add_argument(
        "--dataset", choices=["dns", "dos", "dos_clean", "both"], default="dos_clean",
    ...
    parser.add_argument("--no-autoencoder", action="store_true",
                         help="Skip the TensorFlow autoencoder (useful if TensorFlow isn't installed).")
    ...
    for name in datasets:
        summary[name] = run(name, tune=args.tune, train_autoencoder=not args.no_autoencoder)
